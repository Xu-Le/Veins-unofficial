...
void ContentRSU::initialize(int stage)
{
    ...
    if (stage == 0)
    {
        ...
        // distributeRPeriod += SimTime(1, SIMTIME_NS); // ensure when handle with self message to send the next packet, the previous packet has arrived at another RSU
...
void ContentRSU::handleSelfMsg(cMessage *msg)
{
    ...
    {
        EV << "switched to the scheme in slot " << itRSL->slot << std::endl;
    ...
    case ContentRSUMsgKinds::DISTRIBUTE_V_EVT:
    {
        ...
        ASSERT( distance < 250 );
        int estimatedRate = RoutingUtils::rateTable[distance/10];
        ...
        else
        {
            ...
            {
                EV << "prepare to switch to the scheme in slot " << itRSL->slot << std::endl;
...
void ContentRSU::handleWiredMsg(WiredMessage *wiredMsg)
{
    ...
    EV << "downloader [" << wiredMsg->getDownloader() << "]'s cache end offset updates to " << downloaderInfo->cacheEndOffset << std::endl;

    if (wiredMsg->getControlCode() == WiredMsgCC::LAST_DATA_PACKET) // it is last data packet, indicating prefetch progress has finished
    {
        ...
        if (!rsuSchemeList.empty()) // handle with self transmission scheme
        {
            ...
            dataMsg->setReceiver(itRSL->receiver);
            dataMsg->setDownloader(itRSL->downloader);
            downloaderInfo = downloaders[itRSL->downloader];
            // estimate transmission rate between self and downloader only taking relative distance into consideration
            int distance = RoutingUtils::_length(curPosition, vehicles[itRSL->downloader]->pos);
            ASSERT( distance < 250 );
            int estimatedRate = RoutingUtils::rateTable[distance/10];
...
void ContentRSU::onContent(ContentMessage* contentMsg)
{
    ...
    case ContentMsgCC::CONTENT_REQUEST: // it is a request message from a downloader
    {
        ...
        else
        {
            ...
            DownloaderInfo *downloaderInfo = new DownloaderInfo(contentMsg->getContentSize(), contentMsg->getConsumingRate());
...
SimTime ContentRSU::obtainCooperativeScheme()
{
    ...
    // get scheme item list from STAG
    for (size_t k = 0; k < graph.fluxSchemeList[0].size(); ++k)
    {
    ...
    for (nodeId = 1; nodeId < nodeNum; ++nodeId)
    {
        if (graph.fluxSchemeList[nodeId].empty() == false)
        {
            ...
            for (size_t k = 0; k < graph.fluxSchemeList[nodeId].size(); ++k)
            {
                STAG::FluxScheme &fluxScheme = graph.fluxSchemeList[nodeId][k]; // alias
                schemeItemList[nodeIdMap[nodeId]].push_back(SchemeTuple(fluxScheme.slot, nodeIdMap[fluxScheme.dest], nodeIdMap[fluxScheme.downloader], fluxScheme.flow));
            }
        }
    }
...
void ContentRSU::predictLinkBandwidth()
{
    ...
    std::vector<Coord> nodeSpeed(nodeNum, Coord::ZERO);
    std::vector<Coord> nodePos(nodeNum, Coord::ZERO);
    nodeSpeed[0] = Coord::ZERO; // RSU has no speed since it cannot move
    nodePos[0] = curPosition;   // node 0 is RSU
    for (nodeId = 1; nodeId < nodeNum; ++nodeId) // current position of vehicles
    {
        nodeSpeed[nodeId] = vehicles[nodeIdMap[nodeId]]->speed;
        nodePos[nodeId] = vehicles[nodeIdMap[nodeId]]->pos;
    }
    ...
    for (int j = 0; j < slotNum; ++j)
    {
        ...
        for (srcNode = 0; srcNode < nodeNum-1; ++srcNode)
        {
            for (dstNode = srcNode+1; dstNode < nodeNum; ++dstNode)
            {
                int dist = RoutingUtils::_length(nodePos[srcNode], nodePos[dstNode]);
                if (dist >= 250)
                    itLT->bandwidth[j] = 0;
                else
                    itLT->bandwidth[j] = 128*RoutingUtils::rateTable[dist/10];
                ++itLT;
            }
        }
        for (nodeId = 1; nodeId < nodeNum; ++nodeId) // update vehicles position, assuming they have uniform motion
            nodePos[nodeId] += nodeSpeed[nodeId];
    }
...
void ContentRSU::printLinkBandwidth()
{
    ...
    {
        EV << itLink->src << " " << itLink->dst << " |";
        for (int j = 0; j < slotNum; ++j)
            EV << " " << itLink->bandwidth[j];
        EV << "\n";
    }
    EV << std::endl;
