...
void STAG::destruct()
{
    ...
    for (std::vector<std::vector<FluxScheme> >::iterator itFSL = fluxSchemeList.begin(); itFSL != fluxSchemeList.end(); ++itFSL)
        itFSL->clear();
}
...
void STAG::_obtainArcPathList()
{
    ...
    itAPL = arcPathList.begin();
    ++itAPL;
    for (int i = 1; itAPL != arcPathList.end(); ++i, ++itAPL)
    {
...
{
    std::vector<FluxScheme> fluxScheme;
    fluxScheme.reserve(slotNum);
    int i = 0;
    size_t k = 0;
    for (i = 0; i < nodeNum; ++i)
        fluxSchemeList.push_back(fluxScheme);
    for (itAPL = arcPathList.begin(); itAPL != arcPathList.end(); ++itAPL)
    {
        std::vector<int> &arcPath = *itAPL; // alias
        for (k = 0; k < arcPath.size()-1; ++k)
        {
            struct Arc *parc = &arcTable[arcPath[k]];
            for (int j = 0; j < slotNum; ++j)
