...
void BaseWaveApplLayer::onBeacon(BeaconMessage* beaconMsg)
{
    ...
    PhyToMacControlInfo *lowLayerInfo = dynamic_cast<PhyToMacControlInfo*>(beaconMsg->getControlInfo());
    DeciderResult80211 *phyDeciderResult = dynamic_cast<DeciderResult80211*>(lowLayerInfo->getDeciderResult());
    ...
    if ( neighbors.find(sender) != neighbors.end() ) // update old record
    {
        ...
        EV << "    receiving power is " << phyDeciderResult->getRecvPower_dBm() << std::endl;
        neighborInfo = neighbors[sender]; // alias for efficiency
        neighborInfo->angle = beaconMsg->getSenderAngle();
        neighborInfo->recvPower = phyDeciderResult->getRecvPower_dBm();
    ...
    else // insert new record
    {
        ...
        EV << "    receiving power is " << phyDeciderResult->getRecvPower_dBm() << std::endl;
        neighborInfo = new NeighborInfo(beaconMsg->getSenderAngle(), phyDeciderResult->getRecvPower_dBm(), beaconMsg->getSenderPos(), beaconMsg->getSenderSpeed(), beaconMsg->getSenderFrom(), beaconMsg->getSenderTo(), simTime());
