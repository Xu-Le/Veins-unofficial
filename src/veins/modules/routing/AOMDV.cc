...
void AOMDV::initialize(int stage)
{
	...
	if (stage == 0)
	{
		...
		dataHeaderBits = 288 - headerLength; // data packets are actually carried over IP
		dataLengthBits = par("dataLengthBits").longValue();
		dataPriority = par("dataPriority").longValue();
		bQueueSize = 0;
		bQueueCap = par("bufferQueueCap").longValue();

		pktTransmitDelay = SimTime((288+dataLengthBits)/6, SIMTIME_US); // assume link layer rate is 6Mb/s
...
#endif
		EV << "forward it to next hop: " << nextHop << "\n";
...
void AOMDV::onLinkBroken(LAddress::L3Type neighbor, DataMessage *dataPkt)
{
	...
	if (rt->flags == RTF_UP && (pathNum > 1 || (pathNum == 1 && rt->pathLookup(neighbor) == nullptr)))
	{
		rt->pathDelete(neighbor);
		LAddress::L3Type nextHop = rt->pathSelect()->nextHop;
		dataPkt->setSenderAddress(myAddr);
