...
simtime_t Decider80211p::processNewSignal(AirFrame* msg)
{
	...
	else
	{
		...
		{
			if (currentSignal.first == nullptr)
			{
...
{
	return signalStates.find(frame) == signalStates.end() ? NEW : signalStates[frame];
}
...
{
	Mapping *sinrMap = nullptr;
	Mapping *snrMap = nullptr;
	...
	double sinrMin = MappingUtils::findMin(*sinrMap, min, max);
	double snrMin = collectCollisionStats ? MappingUtils::findMin(*snrMap, min, max) : 1e6;
	...
	case PACKET_OK_RESULT::NOT_DECODED:
	{
		if (!collectCollisionStats)
		{
	...
	case PACKET_OK_RESULT::COLLISION:
	{
...
{
	return PACKET_OK_RESULT::DECODED; // trick, ignore interference
	/*
	// compute success rate depending on mcs and bw
	double packetOkSinr = NistErrorRate::getChunkSuccessRate(bitrate, BW_OFDM_10_MHZ, sinrMin, lengthMPDU);
	...
	double packetOkSnr = 0.0;
	double headerNoErrorSnr = 0.0;
	...
	if (!collectCollisionStats)
	{
		if (rand > headerNoError)
			return PACKET_OK_RESULT::NOT_DECODED;
	}
	else
	{
		if (rand > headerNoError) // ups, we have a header error. is that due to interference?
		{
			if (rand > headerNoErrorSnr) // no. we would have not been able to receive that even without interference
				return PACKET_OK_RESULT::NOT_DECODED;
			else // yes. we would have decoded that without interference
				return PACKET_OK_RESULT::COLLISION;
		}
	}
	...
	if (!collectCollisionStats)
	{
		if (rand > packetOkSinr)
			...
			return PACKET_OK_RESULT::DECODED;
	}
	else
	{
		if (rand > packetOkSinr) // ups, we have an error in the payload. is that due to interference?
		{
			if (rand > packetOkSnr) // no. we would have not been able to receive that even without interference
				return PACKET_OK_RESULT::NOT_DECODED;
			else // yes. we would have decoded that without interference
				return PACKET_OK_RESULT::COLLISION;
		}
...
simtime_t Decider80211p::processSignalEnd(AirFrame* msg)
{
	...
	bool whileSending = false, notSynchronized = false;
	...
	else
	{
		...
		{
			// if this is not the frame we are synchronized on, we cannot receive it
	...
	else
	{
		...
		else if (whileSending)
		{
			...
			phy->sendControlMsgToMac(new cMessage("Error", Decider80211ControlKinds::RECVWHILESEND));
		}
		else
		{
			...
			if (((DeciderResult80211 *)result)->isCollision())
				phy->sendControlMsgToMac(new cMessage("Error", PACKET_OK_RESULT::COLLISION));
			else if (notSynchronized)
				phy->sendControlMsgToMac(new cMessage("Error", Decider80211ControlKinds::NOT_SYNCHRONIZED));
			else
				phy->sendControlMsgToMac(new cMessage("Error", Decider80211ControlKinds::BITERROR));
		}
		delete result;
	}
