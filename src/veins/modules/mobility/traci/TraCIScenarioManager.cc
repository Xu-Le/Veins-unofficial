...
TraCIScenarioManager::TraCIScenarioManager() : myAddVehicleTimer(0), mobRng(0), connection(0), connectAndStartTrigger(0), executeOneTimestepTrigger(0), world(0), cc(0)
{
}
...
void TraCIScenarioManager::initialize(int stage)
{
	...
	if (stage == 0)
	{
	    EV << "TraCIScenarioManager::initialize() called.\n";
		return;
	}
	...
	EV << "TraCIScenarioManager initialized.\n";
}

void TraCIScenarioManager::finish()
{
    EV << "TraCIScenarioManager::finish() called.\n";
	if (connection)
	{
			...
			if (routeIds.size()==0)
			{
				...
				for (std::list<std::string>::const_iterator i = routes.begin(); i != routes.end(); ++i)
				{
					...
					if (par("useRouteDistributions").boolValue() == true)
					{
						...
						{
							EV << "Omitting route " << routeId << " as it seems to be a member of a route distribution (found '#' in name)" << std::endl;
							continue;
						}
					}
...
{
	return unEquippedHosts.find(nodeId) != unEquippedHosts.end();
}

void TraCIScenarioManager::deleteManagedModule(std::string nodeId)
{
    EV << "TraCIScenarioManager::deleteManagedModule() called.\n";
...
void TraCIScenarioManager::initTraCI()
{
    {
        ...
        if (apiVersion == 12)
            EV << "TraCI server \"" << serverVersion << "\" reports API version " << apiVersion << endl;
        else
...

    {
        ...
        EV << "TraCI reports network boundaries (" << x1 << ", " << y1 << ")-(" << x2 << ", " << y2 << ")" << endl;
        ...
        if ((connection->traci2omnet(netbounds2).x > world->getPgs()->x) || (connection->traci2omnet(netbounds1).y > world->getPgs()->y))
            EV << "WARNING: Playground size (" << world->getPgs()->x << ", " << world->getPgs()->y << ") might be too small for vehicle at network bounds (" << connection->traci2omnet(netbounds2).x << ", " << connection->traci2omnet(netbounds1).y << ")" << endl;
    }
...
void TraCIScenarioManager::executeOneTimestep()
{
	...
	if (targetTime > round(connectAt.dbl() * 1000))
	{
		...
		EV << "Getting " << count << " subscription results" << endl;
...
void TraCIScenarioManager::processSimSubscription(std::string objectId, TraCIBuffer& buf)
{
	...
	for (uint8_t j = 0; j < variableNumber_resp; ++j)
	{
		...
		if (variable1_resp == VAR_DEPARTED_VEHICLES_IDS)
		{
			...
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " departed vehicles." << endl;
			for (uint32_t i = 0; i < count; ++i)
			{
		...
		else if (variable1_resp == VAR_ARRIVED_VEHICLES_IDS)
		{
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " arrived vehicles." << endl;
			for (uint32_t i = 0; i < count; ++i)
			{
				std::string idstring; buf >> idstring;
		...
		else if (variable1_resp == VAR_TELEPORT_STARTING_VEHICLES_IDS)
		{
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " vehicles starting to teleport." << endl;
			for (uint32_t i = 0; i < count; ++i)
			{
				std::string idstring; buf >> idstring;
		...
		else if (variable1_resp == VAR_TELEPORT_ENDING_VEHICLES_IDS)
		{
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " vehicles ending teleport." << endl;
			for (uint32_t i = 0; i < count; ++i)
			{
				std::string idstring; buf >> idstring;
				// adding modules is handled on the fly when entering/leaving the ROI
		...
		else if (variable1_resp == VAR_PARKING_STARTING_VEHICLES_IDS)
		{
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " vehicles starting to park." << endl;
			for (uint32_t i = 0; i < count; ++i)
			{
				std::string idstring; buf >> idstring;
		...
		else if (variable1_resp == VAR_PARKING_ENDING_VEHICLES_IDS)
		{
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " vehicles ending to park." << endl;
			for (uint32_t i = 0; i < count; ++i)
			{
				std::string idstring; buf >> idstring;
		...
		else if (variable1_resp == VAR_TIME_STEP)
		{
			...
			EV << "TraCI reports current time step as " << serverTimestep << "ms." << endl;
...
void TraCIScenarioManager::processVehicleSubscription(std::string objectId, TraCIBuffer& buf)
{
	...
	for (uint8_t j = 0; j < variableNumber_resp; ++j)
	{
		...
		else if (variable1_resp == ID_LIST)
		{
			...
			ASSERT(varType == TYPE_STRINGLIST);
			uint32_t count; buf >> count;
			EV << "TraCI reports " << count << " active vehicles." << endl;
	...
	if (!inRoi)
	{
		if (mod)
		{
			...
			EV << "Vehicle #" << objectId << " left region of interest" << endl;
		}
		else if(unEquippedHosts.find(objectId) != unEquippedHosts.end())
		{
			unEquippedHosts.erase(objectId);
			EV << "Vehicle (unequipped) # " << objectId<< " left region of interest" << endl;
		}
		return;
	}
	...
	if (!mod)
	{
		...
		if (mType != "")
		{
			...
			EV << "Added vehicle #" << objectId << endl;
		}
	}
	else
	{
		...
		for (cModule::SubmoduleIterator iter(mod); !iter.end(); iter++)
		{
			...
			TraCIMobility* mm = dynamic_cast<TraCIMobility*>(submod);
			if (!mm) continue;
			EV << "module " << objectId << " moving to " << p.x << "," << p.y << endl;
