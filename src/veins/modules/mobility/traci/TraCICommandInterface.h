...
public:
	...
	{
	public:
		Vehicle(TraCICommandInterface* traci, std::string nodeId) : traci(traci), nodeId(nodeId) { connection = &traci->connection; }
	...
	Vehicle vehicle(std::string nodeId) { return Vehicle(this, nodeId);}

	// Road methods
	class Road
	{
	public:
		Road(TraCICommandInterface* traci, std::string roadId) : traci(traci), roadId(roadId) { connection = &traci->connection; }
	...
	Road road(std::string roadId) { return Road(this, roadId); }
	...
	{
	public:
		Lane(TraCICommandInterface* traci, std::string laneId) : traci(traci), laneId(laneId) { connection = &traci->connection; }
		...
		double getMaxSpeed();
		double getMeanSpeed();
		void setLaneId(std::string _laneId) { laneId = _laneId; }
	...
	Lane lane(std::string laneId) { return Lane(this, laneId); }

	// Trafficlight methods
	class Trafficlight
	{
	public:
		Trafficlight(TraCICommandInterface* traci, std::string trafficLightId) : traci(traci), trafficLightId(trafficLightId) { connection = &traci->connection; }
	...
	Trafficlight trafficlight(std::string trafficLightId) { return Trafficlight(this, trafficLightId); }
	...
	{
	public:
		Polygon(TraCICommandInterface* traci, std::string polyId) : traci(traci), polyId(polyId) { connection = &traci->connection; }
	...
	Polygon polygon(std::string polyId) { return Polygon(this, polyId); }
	...
	{
	public:
		Poi(TraCICommandInterface* traci, std::string poiId) : traci(traci), poiId(poiId) { connection = &traci->connection; }
	...
	Poi poi(std::string poiId) { return Poi(this, poiId); }
	...
	{
	public:
		Junction(TraCICommandInterface* traci, std::string junctionId) : traci(traci), junctionId(junctionId) { connection = &traci->connection; }
	...
	Junction junction(std::string junctionId) { return Junction(this, junctionId); }
	...
	{
	public:
		Route(TraCICommandInterface* traci, std::string routeId) : traci(traci), routeId(routeId) { connection = &traci->connection; }
	...
	Route route(std::string routeId) { return Route(this, routeId); }
	...
	{
	public:
		GuiView(TraCICommandInterface* traci, std::string viewId) : traci(traci), viewId(viewId) { connection = &traci->connection; }
	...
	GuiView guiView(std::string viewId) { return GuiView(this, viewId); }
