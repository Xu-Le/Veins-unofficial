...
int main(int argc, char *argv[])
{
...

	{
		...
		std::pair<double, int> result = graph.maximumFlow();
		info_log("Final total interrupt time is %f, total received data amount is %d.\n", result.first, result.second);
		// graph.test();
	}
	...
	bool schemeOK = verifyScheme(graph, totalInterruptTime, totalReceivedAmount);
	if (schemeOK == true)
		uncond_log("Congratulations! Total interrupt time is %f, total received data amount is %d.\n", totalInterruptTime, totalReceivedAmount);
	else
